/**
 * @description 用来实现育儿的底部选择的组件，主要接收数据，对外传递数据
...
export default class ChildredState extends Component {
    constructor(props) {
        super(props)
        this.store = this.props.dictStore
        this.childredState = this.store.originalData && this.store.originalData.childredState ? this.store.originalData.childredState : []
        this.state = {
            list: map(this.childredState, item => {
                return {
    ...
    // 设置默认回显方法
    setDefault(value) {
        let index = findIndex(this.childredState, item => item.value == value)
